...
## download the zipped file and unzip file tree
...
featureLabels  <- read.table(file.path("data", "UCI HAR Dataset", "features.txt"))
activityLabels <- unlist(activityLabels$V2)
featureLabels <-tolower( unlist( gsub("(\\()|(\\))|-|,", "", featureLabels$V2) ) )

## Get parts of the testDataSet
rawTestData    <- read.table(file.path("data", "UCI HAR Dataset", "test","X_test.txt"))
subjectTestID  <- read.table(file.path("data", "UCI HAR Dataset", "test","subject_test.txt"))
activityTestID <- read.table(file.path("data", "UCI HAR Dataset", "test","y_test.txt"))
...
rawTrainData    <- read.table(file.path("data", "UCI HAR Dataset", "train","X_train.txt"))
subjectTrainID  <- read.table(file.path("data", "UCI HAR Dataset", "train","subject_train.txt"))
activityTrainID <- read.table(file.path("data", "UCI HAR Dataset", "train","y_train.txt"))
...
write.table(dataSet, file = "./data/dataSet.txt", row.names = FALSE)
